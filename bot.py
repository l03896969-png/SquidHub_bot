...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_broadcast":
        ...
        await query.edit_message_text("📨 Отправь текст для рассылки:", reply_markup=back_admin())
        return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(user_id) and context.user_data.get("create_promo"):
        ...
        try:
            ...
            await update.message.reply_text("❌ Сумма и количество — числа.")
        return
    ...
    if context.user_data.get("enter_promo"):
        ...
        if user_id in p["used_by"]:
            await update.message.reply_text("❌ Ты уже активировал.", reply_markup=profile_menu())
            return
        p["uses"] -= 1
...
def main():
    ...
    logger.info("🤖 SquidHub Bot v7.0 запущен!")
