...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Уже одобрен → главное меню
    if is_allowed(user_id):
        msg = await update.message.reply_text(rnd_greet(), parse_mode="Markdown", reply_markup=main_menu(is_admin=False))
        LAST_MESSAGES.setdefault(user_id, []).append(msg.message_id)
        return

    # Мут (только для неодобренных — чтобы не спамили заявками)
    muted, left = is_muted(user_id)
    if muted:
        await update.message.reply_text(rnd_mute(fmt_time(left)), parse_mode="Markdown")
        return
    ...
    if check_spam(user_id):
        ...
        if level == -1:
            await update.message.reply_text("🚫 *Заблокирован за флуд*", parse_mode="Markdown")
        else:
            await update.message.reply_text(rnd_mute(fmt_time(dur)), parse_mode="Markdown")
        return
    ...
    if user_id in PENDING:
        ...
        # Повторно шлём владельцу на случай, если потерялось
        try:
            ...
            logger.error("Не смог отправить заявку владельцу: " + str(e))
        return
    ...
    try:
        ...
        logger.info("Заявка отправлена владельцу от @" + username + " (" + str(user_id) + ")")
    except Exception as e:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_moderation":
        ...
        text = (
            "╭─ 🛡 *Модерация*\n│\n"
            "│ 🛡 Антифлуд: " + anti + "\n"
            "│ Прогрессивные муты: 5м → 1ч → 24ч → бан\n"
            "╰────────"
        )
        await query.edit_message_text(text, parse_mode="Markdown", reply_markup=moderation_menu())
        return
    ...
    if data == "adm_mutes":
        ...
        for uid in list(MUTED.keys()):
            ...
            if left <= 0:
                MUTED.pop(uid, None)
                continue
            text += "• @" + (u.get("username") or str(uid)) + " — " + fmt_time(left) + "\n"
            keyboard.append([InlineKeyboardButton("🔊 @" + (u.get("username") or str(uid)), callback_data="unmute_" + str(uid))])
    ...
    if data == "adm_warns_list":
        ...
        for uid, cnt in WARNINGS.items():
            u = USERS.get(uid, {})
            text += "• @" + (u.get("username") or str(uid)) + " — " + str(cnt) + "\n"
    ...
    if data == "adm_users":
        ...
        for uid, u in list(USERS.items())[:30]:
            ...
            text += status + " @" + (u.get("username") or str(uid)) + "\n"
    ...
    if data == "create_promo":
        ...
        await query.edit_message_text("🎟 `код сумма количество`", parse_mode="Markdown", reply_markup=back_admin())
        return
    ...
    if data == "menu_phone":
        ...
        u["balance"] += 50
        add_xp(user_id, 10)
        await query.edit_message_text("🎁 *+50 монет*", parse_mode="Markdown", reply_markup=profile_menu())
...
def main():
    ...
    logger.info("🤖 SquidHub Bot v13.1 запущен!")
